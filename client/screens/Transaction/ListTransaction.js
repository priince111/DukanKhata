...
const ListTransaction = ({route, navigation}) => {
  ...
  useEffect(() => {
    ...
    if (isFocused) {
      console.log(
        `transactions in list transactions for customer ${customer.name}`,
        transactions,
      );
      // Balance Calculation
      let total = 0,
      ...
      transactions.forEach(txn => {
        ...
        total += sign * txn.totalAmount;
        if (txn.duplicateAmount) duplicate += sign * txn.duplicateAmount;
        if (txn.originalAmount) original += sign * txn.originalAmount;
      });

      setPendingBalance(total);
      setPendingDuplicateBalance(duplicate);
      setPendingOriginalBalance(original);
      console.log('pending sum is', total);
      console.log('customer.pendingsum', customer.pendingSum);
